...
### Added
- `tools/vd/` — Go CLI for tracking, vendoring, and publishing Claude skills. Introduces `skills.toml` manifest, `skills.lock`, and verbs: `init`, `add`, `list`, `sync`, `update`, `diff`, `doctor`, `pin`, `detach`, `remove`, `build`, `cache clean`.
- `marketplace.json` and `plugin.json` are now regenerated by `vd build` in default bundle mode (byte-equal to current live files; no consumer-visible change).
- `.agents/` symlink directory emitted by `vd build` for agent-context tooling.
- Two upstream skills onboarded as proof of end-to-end workflow: `browserbase/skills/browser` and `browserbase/skills/browser-trace`.
