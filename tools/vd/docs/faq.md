...
`vd build` (via the `agents` emitter) creates a `.agents/` directory at the repo root. Each entry is a relative symlink pointing at the corresponding `skills/<name>/` directory:
...
This directory is intended for tools that load agent context from a flat directory (e.g. local MCP servers or custom agent runners). It mirrors `skills/` without duplicating files.
