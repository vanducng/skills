...
1. **`uiuxdesign`** - Design quality hub: style selection, palettes, font pairing, shadcn/ui + Tailwind, a11y review (ALWAYS FIRST)
...
**Before any design work**, consult the `uiuxdesign` references for the product type: `references/style-taxonomy.md` (named styles, best-for/avoid), `references/palettes-and-fonts.md` (palettes + font pairings by mood/product), `references/tailwind-shadcn.md` (theming + review pass).
...
**AI Image Generation & Vision (`omnimedia` skill)**:
- Generate high-quality images from text prompts (Gemini/Imagen, MiniMax, OpenRouter)
- Analyze images, screenshots, and documents; compare designs and identify inconsistencies
- Read and extract information from design files

**Image Editing (`media-processing` skill)**:
- Remove backgrounds, resize, crop, rotate images (ImageMagick)
- Apply masks and perform advanced image editing

**Screenshot Capture & Analysis (`agent-browser` skill + `omnimedia`)**:
- Capture screenshots or recordings of current UI
- Analyze and optimize existing interfaces
- Compare implementations with provided designs

**Figma Tools**: use Figma MCP if available, otherwise analyze exported images via `omnimedia`

**Google Image Search**: use `WebSearch` tool and `agent-browser` skill to capture screenshots
...
2. **Design Phase**:
   ...
   - Generate/modify real assets with omnimedia skill for images and ImageMagick for editing
   - Generate vector assets as SVG files
   - Always review, analyze and double check generated assets with omnimedia skill.
...
4. **Validation Phase**:
   - Use `agent-browser` skill to capture screenshots and compare
   - Use `omnimedia` skill to analyze design quality
   - Use `media-processing` skill or `omnimedia` skill to edit generated assets
