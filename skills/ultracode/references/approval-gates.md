...
- Start expensive or long-running agent swarms (large `Workflow` fan-outs, many `Task` subagents).
...
- Run targeted tests or linters that don't mutate source.
- Create local drafts under the run root.
- Create packet plans and result notes.
- Run local validation commands (e.g. `bash scripts/validate.sh`).
