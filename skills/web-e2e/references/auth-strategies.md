...
agent-browser open http://localhost:8082/login
agent-browser snapshot -i
agent-browser fill <email-ref> "admin@example.com"
agent-browser fill <password-ref> "dev-seeded-only"
agent-browser press Enter                          # fill never submits - submit explicitly
sleep 2 && agent-browser get url                   # guard: landed past the login route
```

`agent-browser fill` **never presses Enter** - the form does not submit until you `press Enter` (or `click` the submit button) explicitly. Fill every field, then submit once.
...
```bash
agent-browser eval "fetch('/api/v1/auth/login', {
  method: 'POST',
  credentials: 'include',
...
- CSP: `agent-browser eval` runs through CDP `Runtime.evaluate` (via Playwright), which is generally not subject to page CSP; the in-page `fetch` it issues IS subject to `connect-src`, which same-origin calls satisfy. If your app's CSP is strict and auth depends on this, verify once with a throwaway eval before relying on it.
