...
# must not recursively run ultracook).
#
# Marker: writes {goal-dir}/.ultracook/delegated-to-auto-loop.json so cross-
...
CODEX_FLAG=""
if [ "$ULTRACOOK_RUNTIME_DETECTED" = "codex" ]; then
  CODEX_FLAG="--codex"
fi
...
goal = f"ultracook:{os.environ['ACTION']} for {os.environ['GOAL_SLUG']} until verifier passes"
