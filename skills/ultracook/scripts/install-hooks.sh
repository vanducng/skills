...
START = "# >>> vd:ultracook managed hooks >>>"
END   = "# <<< vd:ultracook managed hooks <<<"
CMD_MON = "bash ~/.agents/skills/ultracook/scripts/codex-monitor-hook.sh"
CMD_CLN = "bash ~/.agents/skills/ultracook/scripts/codex-hook-cleanup.sh"
...
if mode == "detect":
    if present:
        print(f"✓ ultracook Codex hooks already registered in {cfg}")
        sys.exit(0)
    print(f"✗ ultracook Codex hooks NOT registered in {cfg}")
    warn_symlink()
    print("\nAdd this block (or re-run `vd:ultracook install-hooks --apply`):\n")
    print(BLOCK)
    sys.exit(3)
...
if mode == "apply":
    ...
    try:
        ...
        sys.exit(4)
    print(f"✓ appended ultracook hooks to {real} (backup: {bak})")
...
if mode == "uninstall":
    if START in text and END in text:
        ...
        try:
            ...
            sys.exit(4)
        print(f"✓ removed ultracook managed hooks from {real} (backup: {bak})")
        sys.exit(0)
