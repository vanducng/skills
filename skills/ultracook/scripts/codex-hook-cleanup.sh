...
LOG="$HOME/.ultracook/cleanup.log"
mkdir -p "$(dirname "$LOG")"
...
for root in "${SEARCH_ROOTS[@]}"; do
  ...
  while IFS= read -r spec; do
    ...
    # Only sweep when no result.json exists (i.e. ultracook died mid-wait).
    if [ ! -f "$result" ]; then
  ...
  done < <(find "$root" -path '*/.ultracook/cancel.sentinel' -type f 2>/dev/null)
done

if [ "$COUNT" -gt 0 ]; then
  # Optional: surface to user via systemMessage.
  printf '{"hookSpecificOutput":{"systemMessage":"vd:ultracook cleanup: swept %d stale monitor spec(s) from prior runs"}}\n' "$COUNT"
else
  echo '{}'
fi
exit 0
