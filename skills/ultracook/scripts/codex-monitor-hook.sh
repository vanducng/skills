...
#
# This is ultracook's workaround for the absence of a Monitor-tool analog on
...
while IFS= read -r sentinel; do
  ...
  printf '{"hookSpecificOutput":{"additionalContext":"[ultracook-cancel] goal %s was killed (cancel.sentinel present). IF the goal you are currently executing is %s, STOP its loop now and dispatch no further actions (and /goal cancel if under a Codex /goal). If you are executing a DIFFERENT goal, ignore this."}}\n' "$gd_slug" "$gd_slug"
  exit 0
done < <(find . -path '*/.ultracook/cancel.sentinel' -type f 2>/dev/null)
...
try:
    ...
    print(json.dumps({"hookSpecificOutput": {"additionalContext": f"[ultracook-monitor] spec parse failed: {e}"}}))
    sys.exit(0)
...
if time.time() - started_at > timeout:
    json.dump({"status": "timeout", "exit_code": 124, "evidence": f"poll exceeded {timeout}s"},
              open(result_path, "w"))
    print(json.dumps({"hookSpecificOutput": {"additionalContext": f"[ultracook-monitor] {spec.get('action','?')} TIMEOUT after {timeout}s"}}))
    sys.exit(0)
