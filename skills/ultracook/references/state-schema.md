...
Mutable runtime state for a ultracook goal. Updated every iteration via atomic write protocol. Round-trips across context compaction; source of truth for "where are we now?"
...
Sufficient for `vd:ultracook status` to print a one-line summary without re-reading journal files.
