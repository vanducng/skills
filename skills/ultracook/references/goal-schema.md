...
The single source of truth for "what is this ultracook run trying to accomplish?" Written once at intake; can be edited mid-flight (re-read each iteration).
...
| `short_goal` | string | yes | One-line user intent, verbatim from `vd:ultracook "<text>"`. |
