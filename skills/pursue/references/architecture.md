...
So `vd:pursue` is structured in two layers:
...
5. Bash: PURSUE_TARGET_KIND=cluster PURSUE_ACTION_SHAPE=fix-and-ship \
        PURSUE_BRANCH=fix/cron-retry PURSUE_AUTONOMY=semi \
        bash ~/skills/skills/pursue/scripts/init-goal.sh "fix cron retry"
...
loop:
  ...
  next_action = bash ~/skills/skills/pursue/scripts/resolve-next.sh --goal-dir ...
  ...
  bash ~/skills/skills/pursue/scripts/append-journal.sh --action {name} ...
  bash ~/skills/skills/pursue/scripts/update-state.sh < state-patch
```
