...
Patterns for using Claude Code's `Monitor` tool from `vd:pursue` actions that need to wait on long-running external work (CI, GitHub Actions builds, kubectl rollouts) without blocking the session.
