...
The closed set of actions `vd:pursue`'s executor (Phase 3) can dispatch. Each action maps to one existing `vd:*` skill OR a shell command, with optional verifier binding + parallelism + gating hint.
