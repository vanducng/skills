...
Three modes control how often `vd:pursue`'s executor pauses for user confirmation. Mode is set at intake (`goal.yaml.autonomy`) and can be edited in-place mid-flight — the executor re-reads goal.yaml each iteration.
