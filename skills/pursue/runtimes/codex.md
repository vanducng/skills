...
`CLAUDE_*` tools (`AskUserQuestion`, `Skill`, `Monitor`, `Agent`) are NOT available in a Codex session. This file uses Codex equivalents below. If you reached this file from Claude Code, the detector misfired — set `PURSUE_RUNTIME=claude-code` and retry.
...
2. Read the 4 answers, set env vars: `PURSUE_TARGET_KIND`, `PURSUE_ACTION_SHAPE`, `PURSUE_BRANCH`, `PURSUE_AUTONOMY`, `PURSUE_REUSE_WORKTREE` (if `--reuse`).
