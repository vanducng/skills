...
#   1. $PURSUE_RUNTIME override (claude-code | codex | codex-exec)
...
#       PURSUE_RUNTIME=codex.)
#   3. CODEX signal (CODEX_SESSION_ID) → codex, or codex-exec when the explicit
#      exec contract ($PURSUE_EXEC=1) is set. No env var distinguishes
...
case "${PURSUE_RUNTIME:-}" in
  claude-code|codex|codex-exec) echo "$PURSUE_RUNTIME"; exit 0 ;;
esac
...
EXEC=""
case "${PURSUE_EXEC:-}" in 1|true|yes) EXEC="yes" ;; esac

# 2. CLAUDE wins (even when CODEX also set — see header).
if [ -n "$CLAUDE_SIGNAL" ]; then
  if [ -n "$CODEX_SIGNAL" ]; then
    echo "detect-runtime.sh: both CLAUDE_CODE_* and CODEX_SESSION_ID present; assuming claude-code (CODEX_SESSION_ID commonly leaks via inherit=all). Override with PURSUE_RUNTIME=codex if this is a Codex session." >&2
  fi
