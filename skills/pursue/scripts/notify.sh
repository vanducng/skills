...
# the caller.
#
# Used by pursue on terminal=blocked (semi/auto modes) so the user gets paged
...
#   - fallback log:               always available; writes to ~/.pursue/notifications.log
...
LOG="$HOME/.pursue/notifications.log"
mkdir -p "$(dirname "$LOG")"
printf '%s [%s] %s%s\n' \
