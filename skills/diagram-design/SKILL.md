...
**The design system is skinnable.** All colors, typography, and tokens are documented in [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, ...). The default skin is a cool editorial palette (white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted, silver hairlines). Project-specific overrides belong beside the generated artifact under the injected `Visuals:` path, not in the installed skill package.
...
- [ ] **No label mask overlaps a node drawn after it? (Node fill would clip the text - §6 rule 6. Run `python3 <skill-dir>/scripts/verify-geometry.py <file>`.)**
