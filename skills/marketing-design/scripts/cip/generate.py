...
def generate_with_nano_banana(prompt_data, output_dir=None, model_key="flash", aspect_ratio="1:1", logo_image=None, logo_path=None, provider="codex"):
    """Generate image using Codex gpt-image-2 (default) or Gemini Nano Banana
    ...
    Args:
        ...
        logo_image: PIL.Image object of the brand logo for Gemini image-editing mode
        logo_path: file path of the brand logo, attached as a Codex reference image
        provider: "codex" (default), "gemini", or "auto"
    """
    prompt = prompt_data["prompt"]

    # Default engine: Codex gpt-image-2 (ChatGPT subscription), with the brand logo
    # attached as a reference image for compositing. Gemini is the fallback.
    if provider in ("codex", "auto"):
        ...
        filepath = out_dir / f"{brand_slug}-{deliverable_slug}-{timestamp}.png"
        try:
            refs = [logo_path] if logo_path else None
            out = _codex.generate(prompt, filepath, aspect_ratio=aspect_ratio, images=refs)
            print(f"\n✅ Generated: {out} (codex / gpt-image-2{', logo ref' if refs else ''})")
            return str(out)
        except _codex.CodexUnavailable as e:
            print(f"[codex unavailable: {e}] falling back to Gemini...", file=sys.stderr)
...
def generate_cip_set(brand_name, industry, style=None, deliverables=None, output_dir=None, model_key="flash", logo_path=None, aspect_ratio="1:1", provider="codex"):
...
Image Editing Mode:
  ...
  to incorporate your ACTUAL logo into the CIP mockups.
    ...
    parser.add_argument("--provider", choices=["codex", "gemini", "auto"], default="codex",
                        help="Image engine. 'codex' (default) = gpt-image-2 via ChatGPT subscription, logo attached as reference; falls back to Gemini. 'gemini' forces Nano Banana image-editing. For large --set runs, 'gemini' is faster (Codex is one image/turn).")
    ...
    if args.logo:
        ...
        if action == 'generate':
            ...
            print(f"   python ~/.claude/skills/marketing-design/scripts/logo/generate.py --brand \"{args.brand}\" --industry \"{args.industry}\"")
    ...
    if args.set or args.deliverables:
        ...
        if args.prompt_only:
            ...
            filepath = generate_with_nano_banana(
                ...
                logo_path=args.logo, provider=args.provider,
            )
            if args.json:
