...
def generate_logo(prompt, style=None, industry=None, brand_name=None,
                  output_path=None, use_pro=False, aspect_ratio=None, provider="codex"):
    """Generate a logo. Default engine: Codex gpt-image-2; Gemini is the fallback.

    Args:
        provider: "codex" (default), "gemini", or "auto". Codex uses the ChatGPT
                  subscription (run `codex login`); on failure it falls back to Gemini.
    ...
    # Default engine: Codex gpt-image-2 (ChatGPT subscription).
    if provider in ("codex", "auto"):
        try:
            out = _codex.generate(full_prompt, output_path, aspect_ratio=ratio)
            print(f"Logo saved to: {out} (codex / gpt-image-2)")
            return out
        except _codex.CodexUnavailable as e:
            print(f"[codex unavailable: {e}] falling back to Gemini...", file=sys.stderr)

    # Gemini fallback (or --provider gemini)
    if not GEMINI_API_KEY:
        print("Error: GEMINI_API_KEY not set (and Codex unavailable)")
        print("Set GEMINI_API_KEY, or run `codex login` to use the default Codex engine.")
        return None
...
def main():
    ...
    parser.add_argument("--provider", choices=["codex", "gemini", "auto"], default="codex",
                        help="Image engine. 'codex' (default) = gpt-image-2 via ChatGPT subscription; falls back to Gemini. 'gemini' forces Nano Banana.")
    parser.add_argument("--pro", action="store_true", help="Gemini fallback only: use Nano Banana Pro (gemini-3-pro-image-preview)")
