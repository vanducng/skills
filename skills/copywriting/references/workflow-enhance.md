...
1. **Screenshots provided** → Use `vd:omnimedia` skill to analyze and describe issues in detail
2. **Videos provided** → Use `vd:omnimedia` video-analysis to extract relevant copy issues
3. **Scout codebase** → `vd:scout ext` (preferred) or `vd:scout` to find files needing enhancement
