...
1. **Screenshots provided** → Use `vd:omnimedia` skill to analyze conversion issues
2. **Videos provided** → Use `vd:omnimedia` video-analysis for bottleneck identification
3. **URL provided** → Use `web_fetch` tool to fetch and analyze current issues
4. **Scout codebase** → `vd:scout ext` (preferred) or `vd:scout` to find relevant files
5. **Implement** → Use `fullstack-developer` agent to write enhanced copy into code files

## CRO Plan Creation Workflow

Use when creating a structured CRO optimization plan (e.g., via `vd:plan --hard` with CRO-specific requirements).

### Steps

1. **Analyze** → Gather issues from screenshots/videos/URLs using steps above
2. **Activate** `vd:plan` skill for plan structure
