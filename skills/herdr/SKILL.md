...
For setup or state-detection problems, inspect integrations before changing them:
...
Install or update an integration only when the user asks for setup or repair. Use `herdr integration install <agent>` and verify with `herdr integration status`. An integration can improve state or native session reporting; it does not replace the agent's own lifecycle authority.
...
### Pane cannot reach the Cursor API or resolve hosts
...
Use workspace and tab commands for organization. For an isolated coding checkout, run `vd:worktree` first, read its returned `worktreePath`, then create the pane with that path as `--cwd`. Do not use Herdr's native worktree creation for repo work unless the user explicitly requests it, because it bypasses the `vd:worktree` env, port, install, and lifecycle setup.
...
Only start another agent when the user explicitly asks for another agent or parallel work. Default to a sibling pane in the current tab and current working directory. Do not create a workspace, tab, worktree, or different cwd unless the user explicitly requests that topology or location.
...
Replace `right` with `down` when the layout calls for it. For a prepared `vd:worktree`, add `--cwd <worktreePath>`.
