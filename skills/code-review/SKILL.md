---
name: code-review
description: "Review code with a sharp, encouraging voice - inline GitHub PR comments + a tight summary. Reviews on two axes (Spec fidelity + Standards) via parallel subagents. Supports PR (default), pending changes, commit hash, codebase, and --refactor ('does this fit the codebase / is this slop' - local, report-only) modes. Encodes an opinionated review style: severity-prefixed, concise, actionable, no fluff. For the owned deterministic `miucr` CLI (gated reviews, webhooks, MCP), use vd:miucr."
license: MIT
