...
Out of scope for the polished PR path. Spawn `Task(Explore)` subagents per top-level dir; each returns a findings list with file:line:severity:body. Synthesize into a single markdown report under `plans/reports/code-review-<date>-<slug>.md`.
