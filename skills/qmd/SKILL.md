...
`qmd search` requires all tokens to co-occur in a chunk. With 4+ keywords this often returns zero. Workaround: issue per-token searches and aggregate by vote count. (See `vd:opendesign`'s `qmd_vote_search` for a worked example.)
...
- **`vd:opendesign`** — auto-detects qmd and uses it for skill/design-system catalog search.
