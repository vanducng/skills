...
# the block it writes. Local DoD types only - cook owns this gate.
...
#   ci_green / pod_image_matches      not a cook DoD type - use a ship-stage done_when (see vd:devops deploy-verify)
#
...
eval_one() {
  ...
  case "$type" in
    ...
    ci_green|pod_image_matches)
      echo "FAIL (not a cook DoD type - put image/rollout/CI checks on a ship-stage done_when; see vd:devops deploy-verify)"
      ;;
    *)
