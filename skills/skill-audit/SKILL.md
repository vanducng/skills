...
| Parity | same skill across runtimes, `models` | does a skill work as well outside Claude Code? |
...
2. **Cross-runtime error rates are not comparable.** Claude `err_rate` (~4%) counts hook blocks, stale-`Edit` failures, and permission denials; Codex `err_rate` (~0.4%) counts nonzero exec exits only. Compare a skill against its own runtime's baseline, never across runtimes. Same for `interrupts` (Claude text marker, pi `stopReason: aborted`) vs `aborts` (Codex event). Cursor records no tool results and no token usage, so its `tool_errors`, `interrupts` and `tokens` are structurally 0 - read Cursor for invocations and coverage only.
...
5. **"Never used" ≠ useless.** Claude retention truncates (~5 weeks) while Codex keeps months, so a short window under-counts Claude. Knowledge-pack skills auto-trigger rarely by design; `skillmd_reads` (Codex, pi, Cursor) catches model-initiated loads an invocation count misses. Zero invocations against many matching prompts is a *trigger/description* bug, not a dead skill.
...
| pi | `~/.pi/agent/sessions/<slug>/<ts>_<uuid>.jsonl`, subagents at `<session>/<run-id>/run-N/session.jsonl` | `/`-prefixed skill ID in user text, `SKILL.md` reads |
| Cursor | `~/.cursor/projects/<slug>/agent-transcripts/<id>/<id>.jsonl`, subagents at `<id>/subagents/*.jsonl` | `/` or `$` skill ID inside `<user_query>`, `SKILL.md` reads |
