...
## Different-Width Stacked Boxes

When a connector must join two stacked boxes of different widths, make the boxes the same width, or use an L-shaped connector:
...
The connector column for a same-width box is `border_start + 1 + floor(inner_width / 2)` - see Common Mistakes in SKILL.md for the canonical formula.
