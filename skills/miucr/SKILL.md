...
| `--format full\|minimal` | `full` | Review-comment presentation on `--pr`. `full` is the current output (summary section + severity/priority badges). `minimal` drops the `## Code Review Summary` section, **all** shields badges (summary chips **and** the per-finding inline `P3 · bug`), and the visible footer sub-line (the reviewed-head marker moves into a hidden `<!-- Reviewed commit … -->` token so the resolution-sync still finds it), keeping inline findings + the hidden upsert markers. Additionally, under `minimal` a **clean first-attempt** review that the approval policy would approve posts **no summary comment at all — just the green approval** (the comment returns once there are findings or a prior ledger to maintain). Render-only — does not change which findings surface. An out-of-set value is rejected (`flags.invalid_format`, exit 2). |
