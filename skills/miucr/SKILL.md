...
5. **Poll until complete**:
   ...
   - **A clean review you ran yourself cannot clear `REVIEW_REQUIRED` on a PR you authored.** GitHub forbids self-approval, so `--approval clean` on your own PR returns `approve_action: commented` with `approve_reason: self_approve_forbidden` and `reviewDecision` stays `REVIEW_REQUIRED` (`mergeStateStatus: BLOCKED`) even at zero findings. The approval has to come from a differently-authenticated run - the host daemon, the GitHub Action, or another account. Do not keep re-running the CLI: check `approve_reason` once, then wait for that run or escalate to the user. Re-running only re-edits the same summary comment.
