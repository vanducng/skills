...
- For a new canvas or finalized export, use the hook-injected `Visuals:` directory unless the user supplied a path. It already resolves to the active feature's visuals directory; never reconstruct the workbench layout yourself.
- Name new canvases `<descriptive-slug>.tldraw`. Keep final proof images beside the canvas only when the user requests an export or durable visual evidence; temporary screenshots stay temporary.
- Preserve an existing file's location unless the user asks to move it.
- The Canvas API has no documented save or save-as endpoint. Use tldraw's **File → Save** / **Save As** flow for file placement; do not rely on private renderer functions or guessed bridge APIs.
- Before reporting a new or relocated file complete, query `api.getDocs()` and require the intended `filePath` plus `unsavedChanges: false`.
...
`api.getScreenshot(docId, opts?)` captures a JPEG to a temp file and returns `{ filePath, width, height, pageName, viewport, bounds, captureMode }` — a path, not image data, so open the file yourself to inspect it. A returned path alone is not visual verification. `opts.size` is `'small' | 'medium' | 'large' | 'full'` (default `'small'`). `opts.mode` is `'canvas'` (default — just the shapes, framed to their bounds) or `'window'` (the whole app window: canvas plus UI chrome); use `'window'` to see UI a script's `components` override draws outside the canvas. `opts.bounds` (`{ x, y, w, h }` in page coordinates) applies to `'canvas'` mode only. Prefer reading records with `api.getShapes()`; screenshot only when visual placement is uncertain, layout/readability changed, or the user asks for visual proof.
...
3. Place new persistent outputs in the injected `Visuals:` directory through tldraw's save flow.
4. Run the completion gates below.
5. Stop after one successful pass unless a gate fails or the user explicitly asks for more testing.

## Completion gates

Run the smallest applicable set after the final mutation:

1. **Always — target and content:** re-read `api.getDocs()` and `api.getShapes()`. Confirm the intended document, expected IDs/text/counts, and that unrelated shapes were preserved.
2. **Persistent output:** for a new, saved, or relocated canvas, require the intended `filePath`, `unsavedChanges: false`, and a readable file at that path.
3. **Connections:** when arrows carry meaning, inspect `api.getBindings()` and require both endpoint bindings for every semantic arrow; then run `helpers.getLints()` and resolve every unexpected lint.
4. **Visual layout:** when placement, sizing, labels, arrows, or UI changed, capture at least a medium screenshot and inspect the actual image for overlap, clipping, unreadable text, stray shapes, poor framing, and incorrect connector routing. Record inspection alone is insufficient for these tasks.
5. **Durable behavior:** require script status `state: "applied"`; when behavior changes visible UI, also verify the resulting state with a targeted record assertion or a window screenshot.

A text-only or metadata-only edit may skip the screenshot when records prove the result. If a gate fails, fix the issue and rerun that gate plus any gate the fix could affect; do not repeat unaffected checks.
...
Keep summaries tight. Include the doc id/name, final file path when applicable, changed shape ids or script path, and the gates run with their result. If something fails, quote the server error, digest mismatch, or the relevant `.script-workspace/error.log` line.
