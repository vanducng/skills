...
from codex_imagegen import (
    generate_image as generate_codex_image,
    CodexError,
    CodexNotAvailable,
    CodexQuotaExceeded,
)
...
IMAGE_PROVIDER_VALUES = {'auto', 'google', 'openrouter', 'minimax', 'codex'}
...
def find_api_key() -> Optional[str]:
    ...
    2. PROJECT/.claude/skills/omnimedia/.env (skill-specific)
    3. PROJECT/.claude/skills/.env (shared skills)
    4. PROJECT/.claude/.env (project global)
    5. ~/.claude/skills/omnimedia/.env (user skill-specific)
    ...
    if CENTRALIZED_RESOLVER_AVAILABLE:
        ...
        return resolve_env('GEMINI_API_KEY', skill='omnimedia')
...
def validate_model_task_combination(model: str, task: str, provider: str = 'auto') -> None:
    ...
    if task == 'generate':
        ...
        if resolved_provider == 'codex':
            # Codex picks its image model internally via $imagegen.
            # `--model` here passes to `codex exec -m` as the Codex BASE model.
            return
...
def _run_codex_image_branch(
    prompt: str,
    output_file: Optional[str],
    model_arg: Optional[str],
    verbose: bool = False,
) -> Dict[str, Any]:
    """Run Codex CLI image generation; return the standard result-dict shape.

    Result keys match other providers: 'status', 'generated_images', 'model' on success;
    'status', 'error', 'error_kind' on failure (error_kind ∈ codex_unavailable|codex_quota|codex_error).
    """
    out_path = Path(output_file) if output_file else Path('generated.png')
    try:
        produced = generate_codex_image(
            prompt=prompt,
            out_path=out_path,
            model=model_arg,
        )
        return {
            'status': 'success',
            'generated_images': [str(produced)],
            'model': model_arg or 'codex-default',
            'provider': 'codex',
        }
    except CodexNotAvailable as exc:
        return {
            'status': 'error',
            'error': str(exc),
            'error_kind': 'codex_unavailable',
            'provider': 'codex',
        }
...
def batch_process(
    ...
    if requires_google_sdk:
        ...
        if not api_key:
            ...
            if CENTRALIZED_RESOLVER_AVAILABLE:
                ...
                for i, (desc, path) in enumerate(get_env_file_paths('omnimedia'), 2):
    ...
    if task == 'generate' and not files:
        ...
        # Codex branch (explicit --provider codex) or codex-first cascade (--provider auto)
        codex_explicit = (image_provider == 'codex')
        codex_result = None
        if codex_explicit or provider == 'auto':
            codex_model = model if isinstance(model, str) and model.startswith(
                ('gpt-', 'o3', 'o4', 'codex-')) else None
            codex_result = _run_codex_image_branch(
                prompt=prompt,
            ...
            if codex_result.get('status') == 'success':
                if provider == 'auto':
                    print('[auto] using codex', file=sys.stderr)
                result = codex_result
                results.append(result)
                if verbose:
                    print(f"  Status: {result.get('status', 'unknown')}")
                # Fall to outer return at end of batch_process
                return results
            if codex_explicit:
                # No fallback when explicitly requested
                results.append(codex_result)
                if verbose:
                    print(f"  Status: {codex_result.get('status', 'unknown')}")
                return results
            # auto cascade: log and fall through
            kind = codex_result.get('error_kind', 'codex_error')
            if kind == 'codex_unavailable':
                if verbose:
                    print('[auto] codex unavailable, falling through', file=sys.stderr)
            else:
                print(f'[auto] codex {kind}, falling through to next provider',
                      file=sys.stderr)
            # Resolve next provider as if user had said --provider auto with no codex
            image_provider = infer_image_provider(model, 'auto')
...
Examples:
  ...
  %(prog)s --task generate --provider openrouter \\
    ...
    parser.add_argument('--provider', default='auto',
                       choices=['auto', 'google', 'openrouter', 'minimax', 'codex'],
                       help='Image generation provider for --task generate (default: auto). '
                            'auto cascades: codex → google → openrouter → minimax.')
    ...
    if (
        ...
        args.files and
        infer_image_provider(args.model, args.provider) in ['openrouter', 'minimax', 'codex']
    ):
        parser.error("OpenRouter, MiniMax, and Codex image generation currently support prompt-only generation (no input files)")
