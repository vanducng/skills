#!/usr/bin/env python3
"""
Validate omnimedia skill setup and configuration.
...
def resolve_key(var_name):
    ...
    try:
        from resolve_env import resolve_env
        return resolve_env(var_name, skill='omnimedia')
    except ImportError:
...
def find_api_key():
    ...
    if api_key:
        ...
        print_info("python ~/.claude/scripts/resolve_env.py GEMINI_API_KEY --skill omnimedia --verbose")
        return api_key
...
def provide_setup_instructions():
    ...
    print_info("To configure the omnimedia skill:")
    ...
    print("\n4. Debug if needed:")
    print(f"   $ python ~/.claude/scripts/resolve_env.py --show-hierarchy --skill omnimedia")
    print(f"   $ python ~/.claude/scripts/resolve_env.py GEMINI_API_KEY --skill omnimedia --verbose")


def check_codex_cli():
    """Report Codex CLI availability and ChatGPT login status (non-fatal)."""
    print_header("Checking Codex CLI (subscription image gen)")
    ...
    try:
        ver = _sp.run(["codex", "--version"], capture_output=True, text=True, timeout=10)
        print_success(f"codex CLI: {ver.stdout.strip() or 'installed'}")
    except Exception as e:
        print_warning(f"codex --version failed: {e}")

    try:
        login = _sp.run(["codex", "login", "status"], capture_output=True, text=True, timeout=15)
        combined = (login.stdout or "") + (login.stderr or "")
...
def main():
    ...
    print(f"\n{BOLD}Omnimedia Skill - Setup Checker{RESET}")
