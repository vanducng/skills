...
class TestAPIKeyFinder:
    ...
    @patch('gemini_batch_process.resolve_env', create=True)
    def test_find_api_key_not_found(self, mock_resolve_env, monkeypatch):
        ...
        # In the fork the centralized resolver is not on path; the function
        # falls back to dotenv-only resolution.
...
class TestBatchProcessing:
    ...
    @patch('gemini_batch_process.generate_codex_image')
    ...
    def test_batch_process_auto_falls_back_to_openrouter_on_free_tier_error(
        ...
        # Codex must be tried first under --provider auto; simulate quota
        # exhaustion so we cascade to Google → OpenRouter as before.
        mock_codex.side_effect = gbp.CodexQuotaExceeded("rate_limit_exceeded 429")
        mock_find_key.return_value = 'test_key'
