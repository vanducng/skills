...
| Image Gen | `references/image-generation.md` | Imagen 4 and Gemini image model matrix and selection guide, generate_images vs generate_content API differences, aspect ratios, editing and multi-image composition, prompting techniques, safety settings, limitations, and troubleshooting. |
...
- Transcripts are Markdown with metadata (duration, file size, generated date, file name, topics) and per-chunk lines in the form `[HH:MM:SS -> HH:MM:SS] transcript content`.
