...
- Save under the injected `Visuals:` path in a `mockup-{ts}-{app-slug}/` subdir: `index.html`, `screens/*.html`, `assets/theme.css`, `assets/logo/*`. Use a kebab-case app/feature slug so each design has its own subfolder.
