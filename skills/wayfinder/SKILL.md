---
name: wayfinder
description: "Plan work too big for one agent session as a shared map of decision tickets on an issue tracker, resolved one at a time until the way is clear. Use when a loose idea spans many sessions ('this is huge', 'we'll be at this for a while'), when the user says 'chart this', 'wayfinder', 'work the map', or when brainstorm/plan would produce a 12-phase mega-plan. Plans decisions, not builds - hand the cleared way to vd:plan/vd:cook per chunk. Not for work that fits one session (vd:brainstorm + vd:plan cover that)."
license: MIT
...
| `prototype` | HITL | Brainstorm's prototype detour (`skills/brainstorm/references/prototype.md`); artifact linked, never merged | "How should it look/behave?" is the question |
| `grilling` | HITL | `vd:brainstorm --grill` (frontier rounds, Plannotator-first); sharpen terms against `docs/glossary.md` where one exists | The default - the question is a conversation |
...
1. **Name the destination** - grill it down with `vd:brainstorm --grill`. Settled first, because it fixes the scope.
...
**Composes:** `vd:brainstorm --grill` (grilling tickets + charting), `vd:research` (research tickets), brainstorm's prototype detour, `vd:jira` / `gh` (tracker ops), `vd:docs` glossary (term sharpening)
