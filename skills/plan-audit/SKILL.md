...
- If no argument → check `.work/plans/` then `plans/` for entries matching `{YYYYMMDD-HHMM}-*` or `{YYMMDD-HHMM}-*`; sort by date prefix descending, pick the most recent that contains `plan.md`. Print "Auditing most-recent plan: {path}" so the user can confirm.
- If no plan dir is found → error: "No plan dir argument and no recent plan found under `.work/plans/` or `plans/`. Pass a path explicitly."
